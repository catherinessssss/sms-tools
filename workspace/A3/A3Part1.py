...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    ## Your code here
    s1 = int(fs / f1) # number of samples from first sinusoid per period (fs * (1 / f1))
    s2 = int(fs / f2) # number of samples from second sinusoid per period
    lcm = int(s1 * s2 / gcd(s1, s2)) # lease common multiple
     
    X = fft(x[:lcm])
    lcm_left = int(lcm / 2 + 1)
        
    mx = 20 * np.log10(np.abs(X[:lcm_left]))
